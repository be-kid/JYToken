...
contract SimpleToken is ERC20Interface, OwnerHelper {
    ...
    mapping (address => bool) public _personalParticipate;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        checkParticipateExp(msg.sender);
        checkParticipateExp(recipient);
    ...
    function isTokenLock(address from, address to) public view returns (bool lock) { // 토큰이 잠겼는지, 개인락이 걸렸는지 확인한다
        lock = false;
    ...
    function checkParticipateExp(address _who) public { // 계정이 활동한적이 있는지. 처음이면 토큰을 잠근 상태로 설정
        if (_personalParticipate[_who] == false){
            _personalTokenLock[_who] = true;
        }
    }

    function changeTokenLock() onlyOwner public {
        if(_tokenLock == true){
            _tokenLock = false;
        }else{
            _tokenLock = true;
    ...
    function changePersonalTokenLock(address _who) onlyOwner public { // 락 상태를 바꿈. 활동 여부도 참으로
        ...
        _personalParticipate[_who] = true;
    }
}
